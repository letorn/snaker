...
begin
	...
	set msg = '';
	set crecount = 0;
	set establish = if(establish = '' or establish is null, null, establish);
	...
	if categorycode is null or categorycode = '' then
		set msg = concat(msg, ";所属行业编码 不能为空");
		set issuccess = 0;
	end if;
	  
	if naturecode is null or naturecode = '' then
		set msg = concat(msg, ";行业性质编码 不能为空");
		set issuccess = 0;
	end if;
	  
	if scalecode is null or scalecode = '' then
		set scalecode = null;
	end if;
	
	if entrole is null or entrole = '' then
		set entrole = 2;
	end if;
   
	if eid = 0 then
		if (de_account is null or de_account = '') then
	...
	if issuccess = 1 then
		if eid = 0 then
			insert into zcdh_ent_lbs(latitude, longitude) values(lbslat, lbslon);
			select @@identity into lbsid;
			...
			insert into zcdh_user_role_rel (role_id, user_id) values(entrole, eid);
				
		else 
		
			select lbs_id into lbsid from zcdh_ent_enterprise where ent_id = eid limit 1;
		
			select count(1) into lbscount from zcdh_ent_lbs where lbs_id = lbsid;
			
			if lbscount = 0 then 
				insert into zcdh_ent_lbs (latitude, longitude) values(lbslat, lbslon);
				select @@identity into lbsid;
			else 
				update zcdh_ent_lbs set latitude = lbslat, longitude = lbslon where lbs_id = lbsid;
			end if;
		...
		select count(1) into crecount from zcdh_ent_credentials where ent_id = eid;

		if crecount = 0 then 
			if (orgains is null or orgains = '') and license is not null and license != '' then 
				insert into zcdh_ent_credentials(ent_id, type, credential_num) values(eid, '001', license);
			elseif (license is null or license = '') and orgains is not null and orgains != '' then 
				insert into zcdh_ent_credentials(ent_id, type, credential_num) values(eid, '002', orgains);
			end if;
		else
			if (orgains is null or orgains = '') and license is not null and license != '' then 
				update zcdh_ent_credentials set	type = '001', credential_num = license where ent_id = eid;
			elseif (license is null or license = '') and orgains is not null and orgains != '' then 
				update zcdh_ent_credentials set type = '002', credential_num = orgains where ent_id = eid;
			else 
				delete from zcdh_ent_credentials where ent_id = eid;
			end if;
		end if;
