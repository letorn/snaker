...
begin
	...
	if headcount is null or headcount = '' or headcount = -1 then 
		set headcount = 10;
		set is_several = 1;
	end if;
	
	if age is null or age = '' or age = -1 then 
		set age = "00-99";
	end if;
	...
	if salary is null or salary = '' or salary = -1 then
		set salary = 0;
	end if;
	
	if experiencecode is null or experiencecode = '' then
		set experiencecode = '005.009';
	end if;
	
	if educationcode is null or educationcode = '' then 
		set educationcode = '004.011';
	end if;
	
	if dataentkey is null or dataentkey = '' then
	...
	if issuccess = 1 then
		...
		if pid = 0 then 
			...
			select @@identity into pid;

		else 
			select lbs_id into lbsid from zcdh_ent_post where id = pid limit 1;
