...
public class DataService {
	...
	public boolean postEnterprise(Long[] ids, String[] codes) {
		...
		if (ids.length > 0)
			sqls.add("(select id,name,category_code,nature_code,scale_code,tag,establish,introduction,address,website,area_code,lbs_lon,lbs_lat,orgains,license,contacter,public_contact,phone,fax,mobile,email,qq,data_src,data_key,update_date,create_date,account,role from vi_enterprise where syn_status=-1 and id in(" + StringUtils.join(ids, ",") + "))");
		for (String code : codes)
			sqls.add("(select id,name,category_code,nature_code,scale_code,tag,establish,introduction,address,website,area_code,lbs_lon,lbs_lat,orgains,license,contacter,public_contact,phone,fax,mobile,email,qq,data_src,data_key,update_date,create_date,account,role from vi_enterprise where syn_status=-1 and category_code=? limit 10)");
